...
b0005_charge = generate_charge_dataset(b0005_data.B0005, 'B0005');
b0006_charge = generate_charge_dataset(b0006_data.B0006, 'B0006');
b0007_charge = generate_charge_dataset(b0007_data.B0007, 'B0007');
b0018_charge = generate_charge_dataset(b0018_data.B0018, 'B0018');
...
b0005_discharge = generate_discharge_dataset(b0005_data.B0005, 'B0005');
b0006_discharge = generate_discharge_dataset(b0006_data.B0006, 'B0006');
b0007_discharge = generate_discharge_dataset(b0007_data.B0007, 'B0007');
b0018_discharge = generate_discharge_dataset(b0018_data.B0018, 'B0018');
...
function charge_dataset = generate_charge_dataset(bat_struct, battery_name)
    num_of_cycles = length(bat_struct.cycle);
    battery_data = table;
    for i = 1:num_of_cycles
        if strcmp(bat_struct.cycle(i).type,'charge')
            len = length(bat_struct.cycle(i).data.Voltage_measured);
            bat_temp = table(repmat({battery_name}, len, 1),...
                repmat({bat_struct.cycle(i).type}, len, 1),...
                repmat({bat_struct.cycle(i).ambient_temperature}, len, 1),...
                repmat({datetime(bat_struct.cycle(i).time)}, len, 1),...
                ...
                'VariableNames', {'Battery','Type','Ambient_temperature','Time','Voltage_measured', 'Current_measured', 'Temperature_measured','Current_charge','Voltage_charge','Charge_Time'});
...
function discharge_dataset = generate_discharge_dataset(bat_struct,battery_name)
    num_of_cycles = length(bat_struct.cycle);
    battery_data = table;
    for i = 1:num_of_cycles
        if strcmp(bat_struct.cycle(i).type,'discharge')
            ...
            bat_temp = table(repmat({battery_name}, len, 1),...
                ...
                bat_struct.cycle(i).data.Time',...
                repmat(bat_struct.cycle(i).data.Capacity, len, 1),...
                'VariableNames', {'Battery','Type','Ambient_temperature','Time','Voltage_measured', 'Current_measured', 'Temperature_measured','Current_load', 'Voltage_load','Discharge_time', 'Capacity'});
            battery_data = [battery_data;bat_temp];
        end
    end
