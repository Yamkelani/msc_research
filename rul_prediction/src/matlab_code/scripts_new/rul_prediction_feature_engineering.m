...
'cv_time','soc','charge_time','average_room_temperature','average_cell_temperature',...
'max_discharge_voltage','max_discharge_current', 'cycle_discharge_time','max_room_temperature',''};
...
for i = 1:length(discharge_data_files_list)
    ...
    rul_features = generate_features(rul_features, charge_data, discharge_data);
  

end


function dateset = generate_features(features, battery_charge_data, battery_discharge_data) 
    ...
    charge_cc_time = battery_charge_data.Time(battery_charge_data.Measured_cell_voltage <= max_charge_voltage);
    ...
    max_voltage_time = battery_charge_data.Time(battery_charge_data.Measured_cell_voltage == max_charge_voltage);
