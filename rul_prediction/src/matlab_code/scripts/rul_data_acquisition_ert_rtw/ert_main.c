...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T)
    rul_data_acquisition_M->Timing.clockTick0;
  rul_data_acquisition_step();
...
int main(void)
{
  ...
  ;
  rtmSetErrorStatus(rul_data_acquisition_M, 0);
  ...
  rul_data_acquisition_initialize();
  globalInterruptDisable();
  globalInterruptEnable();

  /* External Mode initialization */
  errorCode = extmodeInit(rul_data_acquisition_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(rul_data_acquisition_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(rul_data_acquisition_M);
  enableTimer0Interrupt();
  globalInterruptEnable();
  while (runModel) {
  ...
  /* Terminate model */
  rul_data_acquisition_terminate();
