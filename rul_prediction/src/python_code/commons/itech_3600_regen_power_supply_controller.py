    ...
    def battery_charger(self, **charging_configs):
        try:
            ...
            if emulator_status == 0:
                ...
                with open(f"{charging_configs['charge_data_path']}/cell_{test_cell}_dicharging_data_{datetime.datetime.now()}.json", "w") as outfile:
    ...
    def battery_discharge(self, **discharge_configs):
        try:
            ...
            if emulator_status == 0:
                ...
                print(f"Discharging voltage is: {voltage}").strip("/n")
            ...
            with open(f"{discharge_configs['discharge_data_path']}/cell_{test_cell}_dicharging_data_{datetime.datetime.now()}.json", "w") as outfile:
...
def main(**kwaargs):
    ...
    config_path = "C:/Users/yamzi/source_code/msc_research/rul_prediction/src/python_code/configs/BAT_CELL_BL26650.json"
    battery_test_path = "C:/Users/yamzi/source_code/msc_research/rul_prediction/src/datasets"
    battery_test_cell = config['test']['Battery_cell']

    with open(config_path, 'r') as config_file:
        config = json.load(config_file)

    solar_emulator = PVStorageEmulator(config['test']['test_inst_addr'])
    ...
    solar_emulator.battery_charger(
                                   charge_data_path = battery_test_path,
